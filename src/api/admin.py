...
async def get_proxy_config(token: str = Depends(verify_admin_token)):
    ...
    rotation_status = await proxy_manager.get_rotation_status()
    return {
        "success": True,
        "config": {
            "enabled": config.enabled,
            "proxy_url": config.proxy_url,
            "proxy_pool_enabled": config.proxy_pool_enabled,
            "proxy_pool": config.proxy_pool or "",
            "rotation_mode": config.rotation_mode,
            "rotate_every_requests": config.rotate_every_requests,
            "rotate_every_seconds": config.rotate_every_seconds,
            "rotate_every_failures": config.rotate_every_failures,
            "sync_browser_proxy": config.sync_browser_proxy,
...
async def update_proxy_config_alias(
    ...
    try:
        await proxy_manager.update_proxy_config(
            ...
            proxy_pool_enabled=request.proxy_pool_enabled,
            proxy_pool=request.proxy_pool,
            rotation_mode=request.rotation_mode,
            rotate_every_requests=request.rotate_every_requests,
            rotate_every_seconds=request.rotate_every_seconds,
            rotate_every_failures=request.rotate_every_failures,
            sync_browser_proxy=request.sync_browser_proxy,
...
async def update_captcha_config(
    ...
    if captcha_method == "browser":
        try:
            ...
            service = await BrowserCaptchaService.get_instance(db, proxy_manager)
...
async def test_captcha_score(
    ...
    browser_proxy_url = captcha_config.browser_proxy_url or ""
    proxy_cfg = await proxy_manager.get_proxy_config() if proxy_manager else None
    ...
    try:
        ...
        if captcha_method == "browser":
            ...
            if token_value:
                ...
                actual_proxy_url = (
                    fingerprint.get("proxy_url")
                    if isinstance(fingerprint, dict)
                    else None
                )
                verify_proxy_used = bool(actual_proxy_url)
                if verify_proxy_used:
                    verify_proxy_url = actual_proxy_url
                    verify_proxy_source = (
                        "request_proxy"
                        if proxy_cfg and proxy_cfg.sync_browser_proxy and proxy_cfg.enabled
                        else "captcha_browser_proxy"
                    )
                else:
                    verify_proxy_source = "browser_direct"
