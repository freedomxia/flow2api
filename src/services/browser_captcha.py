...
def _find_system_chromium_path() -> Optional[str]:
    """查找系统已安装的 Chromium/Chrome 可执行文件"""
    candidates = [
...
def _ensure_browser_installed() -> bool:
    """确保 chromium 浏览器已安装"""
    system_browser_path = _find_system_chromium_path()
    if system_browser_path:
        debug_logger.log_info(f"[BrowserCaptcha] 使用系统 chromium: {system_browser_path}")
        return True
...
if IS_DOCKER and not HAS_DISPLAY:
    debug_logger.log_warning("[BrowserCaptcha] 检测到 Docker 环境但未配置 DISPLAY，有头浏览器打码不可用")
    print("[BrowserCaptcha] ⚠️ 检测到 Docker 环境但未配置 DISPLAY，有头浏览器打码不可用")
    print("[BrowserCaptcha] 请为容器配置 Xvfb/显示环境，或改用第三方打码服务")
else:
...
class TokenBrowser:
    ...
    def __init__(self, token_id: int, user_data_dir: str, db=None, proxy_manager=None):
    ...
    async def _create_browser(self) -> tuple:
        ...
        try:
            ...
            if self.proxy_manager:
                candidate_proxy_url = await self.proxy_manager.get_browser_proxy_url(bind_if_missing=True)
            elif self.db:
            ...
            debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} 加载浏览器代理失败: {e}")
...
class BrowserCaptchaService:
    ...
    def __init__(self, db=None, proxy_manager=None):
        self.db = db
        self.proxy_manager = proxy_manager
    ...
    @classmethod
    async def get_instance(cls, db=None, proxy_manager=None) -> 'BrowserCaptchaService':
        if cls._instance is None:
            async with cls._lock:
                if cls._instance is None:
                    cls._instance = cls(db, proxy_manager)
            ...
            if proxy_manager is not None:
                cls._instance.proxy_manager = proxy_manager
            if db is not None or proxy_manager is not None:
                async with cls._instance._browsers_lock:
                    for browser in cls._instance._browsers.values():
                        if proxy_manager is not None:
                            browser.proxy_manager = proxy_manager
                        if db is not None:
                            browser.db = db
    ...
    def _check_available(self):
        ...
        if IS_DOCKER and not HAS_DISPLAY:
            raise RuntimeError(
                "有头浏览器打码需要可用的 DISPLAY。"
                "请为 Docker 容器配置 Xvfb/显示环境，或使用第三方打码服务"
            )
